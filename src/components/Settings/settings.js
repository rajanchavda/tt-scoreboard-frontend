...
const Settings = ({ settings, gameId }) => {
  ...
  return (<>
    <div id='settings'>
      <div className='absolute right-[20px] top-[20px] w-[20px] h-[20px] cursor-pointer' onClick={() => setShowModal(true)}>
      ...
      >
        <div className='flex justify-end cursor-pointer' onClick={closeModal}>X</div>
        <div className='p-[20px] flex-grow'>

          <ul className='ul'>
            <li className='mb-[20px] flex items-center'>
              <span className='mr-[30px] text-[20px]'>Game Type</span>
              <select className='p-[10px]' value={updatedSettings?.gameType} onChange={($event) => onSettingUpdate('gameType', $event.target.value)}>
            ...
            <li className='mb-[20px] flex items-center'>
              ...
              <div className='p-[10px] flex'>
                <div className='flex mr-[10px] items-center'>
                  ...
                  <input className='w-[62px] h-[30px] p-[10px] border border-[grey]'
                    type='number'
                    value={updatedSettings?.winPoints?.singles}
                    onChange={($event) => { onSettingUpdate('winPoints', { ...updatedSettings?.winPoints, singles: Number($event.target.value) }) }}
                  />
                </div>

                <div className='flex mr-[10px] items-center'>
                  <span className='mr-[10px]'>Doubles</span>
                  <input className='w-[62px] h-[30px] p-[10px] border border-[grey]' type='number'
                    value={updatedSettings?.winPoints?.doubles}
                    onChange={($event) => onSettingUpdate('winPoints', { ...updatedSettings?.winPoints, doubles: Number($event.target.value) })}
                  />
                </div>
              </div>
            </li>
        ...
        <div className='flex justify-end'>
          <button onClick={callUpdateSettings} className='border border-blue-700 p-[10px] bg-[blue] text-[white] w-[100px] rounded-[6px]'>Save</button>
        </div>
      </Modal>
    </div>
  </>
